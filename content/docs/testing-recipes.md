...
Обычно, чтобы это сделать, следует использовать пару `beforeEach` и `afterEach` блоков, они всегда будут исполняться и изолировать тесты друг от друга:
...
Как правило, вы хотите проверить правильность результата рендеринга компонента с определёнными пропсами. Рассмотрим простой компонент, который рендерит сообщение на основе пропса:
...
Фреймворки, такие как Jest позволяют сохранять «снимки» данных используя [`toMatchSnapshot` / `toMatchInlineSnapshot`](https://jestjs.io/docs/ru/snapshot-testing). С их помощью мы можем «сохранить» результат рендера компонента и убедиться, что изменение в нем явно отражено в снимке.
