...
* `yarn build` создаёт директорию `build`, в которую помещает готовые пакеты.
...
Для этого, прежде всего, нужно запустить команду `yarn build`. Эта команда компилирует и выгружает в директорию `build` готовую сборку, а также создаёт в директорию `build/packages` npm-пакеты.

Для того чтобы проверить работоспособность изменений, проще всего запустить команду `yarn build react/index,react-dom/index --type=UMD`. После её завершения нужно открыть файл `fixtures/packaging/babel-standalone/dev.html`. Этот файл запускает скомпилированный `react.development.js` из директории `build`.
...
После каждого запуска в корневой директории React команды `yarn build` пакеты React будут обновляться в `node_modules` вашего тестового проекта.
