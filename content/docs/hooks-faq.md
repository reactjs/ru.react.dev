...
Обычно, вы не должны мутировать локальное состояние в React. Однако, в качестве трюка, вы можете использовать увеличивающийся счётчик, чтобы заставить компонент повторно рендериться, если состояние не изменилось:
