...
Предыдущие пропсы или состояние получают в двух случаях.

Иногда предыдущие пропсы нужны чтобы **сбросить эффект.** Например, когда создаете эффект который подписывается на сокет на основе пропса `userId`. Если пропс `userId` меняется — отпишитесь от _previous_ `userId` и подпишитесь на _next_. Это легко сделать:
...
В этом примере, если `userId` изменится с `3` на `4`, сначала выполнится `ChatAPI.unsubscribeFromSocket(3)`, а затем `ChatAPI.subscribeToSocket(4)`. Не получайте "pervious" `userId`, поскольку функция очистки перехватит его в замыкании.

В других случаях нужно **корректировать состояние на основе изменений в пропсах или других состояниях**. Это признак наличия дублирующего или избыточного состояния и требуется не часто. В редких случаях, когда нужен этот паттерн, можно [хранить предыдущее состояние или пропсы в состоянии и обновлять их во время рендеринга](#how-do-i-implement-getderivedstatefromprops).

Мы уже предлагали пользовательский Hook под названием `usePrevious` для хранения предыдущего значения. Однако мы обнаружили, что большинство случаев использования попадают в два описанных выше паттерна. В других случаях, можно [хранить значение в ссылке](#is-there-something-like-instance-variables) и вручную обновлять его, когда это нужно. Избегайте чтения и обновления ссылок во время рендеринга, поскольку это делает поведение компонента трудно предсказуемым и понятным.
