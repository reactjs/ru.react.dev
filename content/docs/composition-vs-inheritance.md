...
## Изолирование {#containment}

Некоторые компоненты не знают заранее свои дочерние компоненты. Это особенно характерно для таких компонентов, как `Sidebar` или `Dialog`, которые представляют из себя как бы «контейнер».
