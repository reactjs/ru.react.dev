...
Сигнатура идентична хуку `useEffect`, но этот хук запускается синхронно _перед_ всеми изменениями DOM. Используйте его, чтобы внедрить стили в DOM перед чтением макета в [`useLayoutEffect`](#uselayouteffect).  Поскольку область действия этого хука ограничена, этот хук не имеет доступа к `refs` и не может планировать обновления.

> Примечание:
>
> `useInsertionEffect` должен быть ограничен авторами `css-in-js` библиотек. Вместо этого лучше использовать [`useEffect`](#useeffect) или [`useLayoutEffect`](#uselayouteffect).
