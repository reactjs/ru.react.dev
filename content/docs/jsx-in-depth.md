...
Слова в типах пишутся с большой буквы и указывают, что JSX-тег ссылается на React-компонент. Эти теги компилируются в прямые ссылки на именованные переменные, поэтому, если вы используете JSX-выражение `<Foo />` , `Foo` должен быть в области видимости. 
...
Когда вы передаёте литерал строки значение будет в HTML будет неэкранированным. Поэтому эти два JSX выражения будут эквивалентны:
...
В JSX-выражениях, содержание между открывающими и закрывающими тегами передаётся с помощью специального пропа: `props.children`. Существует несколько способов передать дочерние компоненты:
...
Это корректный JSX-код, из-за этого `props.children` в `MyComponent` будет строкой `"Привет, мир!"`. HTML не экранирован, поэтому можно писать на JSX также, как если бы вы писали на обычном HTML:

```html
<div>This is valid HTML &amp; JSX at the same time.</div>
```

JSX удаляет пробелы в начале и в конце строки. Он также удаляет пустые строки. Новая строка, примыкающая в тегу будет удалена; новые строки, находящиеся посередине строковых литералов, сжимаются в один пробел. Всё это отрендерится в один и тот же вид:
...
Дочерние компоненты, передаваемые пользовательскому компоненту, могут быть чем угодно, до тех пор, пока компонент не преобразует их во что-нибудь, что React сможет понять перед рендерингом. Это не распространнённый способ, но им можно воспользоваться, если необходимо расширить возможности JSX.
...
Это может быть полезным для рендеринга по условию. Этот JSX рендерит `<Header />`, если `showHeader` равняется `true`:
