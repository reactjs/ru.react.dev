...
Для рендеринга React-элемента, сперва передайте DOM-элемент в [`ReactDOM.createRoot()`](/docs/react-dom-client.html#createroot), далее передайте с React-элементом в `root.render()`:
