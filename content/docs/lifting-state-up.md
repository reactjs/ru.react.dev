...
В React это обычно решается путём создания «управляемого» компонента. Точно так же, как DOM-элемент `<input>` принимает атрибуты `value` и `onChange`, то и пользовательский `TemperatureInput` принимает оба пропса `temperature` и `onTemperatureChange` от своего родителя `Calculator`.
...
Прежде чем погрузиться в изменения `Calculator`, давайте вспомним сделанные изменения в компоненте `TemperatureInput`. Мы удалили из него локальное состояние, и вместо использования `this.state.temperature` теперь используем `this.props.temperature`. Вместо вызова `this.setState()`, когда мы хотим внести изменения, теперь вызываем `this.props.onTemperatureChange()`, который передаётся компонентом `Calculator`:
...
Когда вы видите, что в UI что-то не так, вы можете использовать [React Developer Tools](https://github.com/facebook/react-devtools) для проверки пропсов и навигации по дереву компонентов вверх до тех пор, пока не найдёте тот компонент, который отвечает за обновление состояния. Это позволяет отследить источник багов:
