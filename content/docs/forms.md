...
Опция "Кокос" выбрана по умолчанию из за установленного атрибута `selected`. React вместо этого атрибута использует `value` на корневом теге `select`. Это удобно в управляемом компоненте потому что обновлять значение нужно только в одном месте (`state`). Пример:
