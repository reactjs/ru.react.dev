...
Второй необязательный параметр в `setState()` - функция колбэк, которая вызывается после того, как будет выполнен `setState` и рендер компоненты завершиться. В общем случае мы рекомендуем использовать `componentDidUpdate()` для такой логики вместо колбека.
