...
Второй параметр в `setState()` - необязательный колбэк, вызываемый после выполнения `setState` и повторного рендера компонента. В большинстве случаев для такой логики вместо колбэка мы рекомендуем использовать `componentDidUpdate()`/.
