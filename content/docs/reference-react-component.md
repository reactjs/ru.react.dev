...
React позволяет определять компоненты как классы или функции. В настоящее время классовые компоненты имеют больше возможностей. Они разобраны на этой странице. Чтобы определить такой компонент, необходимо расширить `React.Component`:
...
Каждый компонент имеет несколько «методов жизненного цикла». Переопределение этих методов позволяет выполнять код в конкретное время в процессе. **Вы можете использовать [эту диаграмму жизненного цикла](http://projects.wojtekmaj.pl/react-lifecycle-methods-diagram/) как шпаргалку.** Далее на странице **полужирным шрифтом** выделены самые распространённые методы жизненного цикла.

#### Монтирование {#mounting}

Эти методы вызываются в следующем порядке при создании экземпляра компонента и его вставки в DOM:
...
Обновление происходит при изменении пропсов или состояния. Эти методы вызываются в следующем порядке при повторном рендере компонента:
...
Эти методы вызываются если произошла ошибка в процессе рендеринга, методе жизненного цикла или конструкторе любого дочернего компонента.

- [`static getDerivedStateFromError()`](#static-getderivedstatefromerror)
- [`componentDidCatch()`](#componentdidcatch)

### Другие API {#other-apis}
...
Конструктор компонента React вызывается до того, как компонент будет примонтирован. Вначале конструктора необходимо вызывать `super(props)`. Если это не сделать, `this.props` не будет определен. Это может привести к багам.
...
`componentDidMount()` вызывается сразу после монтирования (компонент вставлен в DOM). В этом методе должны происходить действия, которые требуют наличия DOM-узлов. Это хорошее место для создания сетевых запросов.
...
Метод позволяет работать с DOM при обновлении компонента. Также он подходит для выполнения сетевых запросов, которые необходимы для сравнения текущих пропсов с предыдущими. Запрос можно не делать, если пропсы не изменились.
...
В `componentDidUpdate()` может быть доступен третий параметр `snapshot`, только в том случае, если реализован метод жизненного цикла `getSnapshotBeforeUpdate()` (используется редко).
...
`componentWillUnmount()` вызывается сразу перед размонтированием и удалением компонента. В этом методе выполняется необходимый сброс: отмена таймеров, сетевых запросов и подписок, созданных в `componentDidMount()`.
...
Если вы уверены, что хотите написать его вручную, вы можете сравнить `this.props` с `nextProps` и `this.state` с `nextState`. Верните `false` что бы пропустить обновление React. Возвращение `false` не предотвращает повторный рендер дочерних компонентов при изменении *их* состояния.
...
`getDerivedStateFromProps` вызывается сразу перед вызовом метода `render`, как при начальном монтировании, так и при последующих обновлениях. Он должен вернуть объект для обновления состояния или null, чтобы ничего не обновлять.

Этот метод существует для [редких случаев](/blog/2018/06/07/you-probably-dont-need-derived-state.html#when-to-use-derived-state), когда изменения в пропсах не сразу влияют на состояние. Например, это подойдёт для реализации компонента `<Transition>`, который сравнивает свои предыдущие и следующие дочерние компоненты, чтобы решить, какой из них нужно анимировать.
...
Если вы хотите переиспользовать код между `getDerivedStateFromProps()` и другими методами класса, извлекая чистые функции пропсов и состояния компонента вне определения класса.
...
`UNSAFE_componentWillMount()` вызывается сразу перед монтированием. Он вызывается перед `render()`, поэтому синхронный вызов `setState()` в этом методе не вызовет дополнительный рендер. Для инициализации состояния мы рекомендуем использовать `constructor()`.
...
`state` – ссылка на состояние компонента при изменении. Она не должна мутировать. Изменения должны быть в виде нового объекта на основе входных данных из `state` и` props`. Предположим, что мы хотели бы увеличить значение состояния с помощью `props.step`:
...
В качестве первого аргументы `setState()`, вместо функции, вы можете передать объект:
...
`defaultProps` можно определить как свойство самого класса компонента, которое позволяет установить пропсы класса по умолчанию. Это используется для неопределённых пропсов, но не для пропсов со значением null. Например:
...
Состояние содержит данные, конкретные для этого компонента. Они могут измениться со временем. Состояние определяется пользователем. Оно должно быть простым объектом JavaScript.

Вам не нужно вставлять в состояние значение, которое не используется для рендера или потока данных (например, идентификатор таймера). Такие значения можно определить как поля экземпляра компонента.
