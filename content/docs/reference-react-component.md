...
Второй необязательный параметр в `setState()` - колбэк, который вызывается после того, как будет выполнен `setState` и рендер компоненты завершиться. В общем случае мы рекомендуем использовать `componentDidUpdate()` для такой логики вместо колбека.
