...
Может быть, у вас установлена версия `react-dom` (< 16.8.0) или `react-native` (< 0.59), которая пока не поддерживает хуки. Вы можете выполнить `npm ls react-dom` или `npm ls react-native` в директории приложения, чтобы посмотреть, какую версию вы используете. Если у вас более одной версии, это также может привести к проблемам (подробнее об этом ниже).
...
Если вы используете Node для управления пакетами, можете проверить копии пакета, находясь в директории проекта:
...
Эта проблема также может возникнуть при использовании команды `npm link` или ей подобной. В таком случае ваш бандлер может «увидеть» два пакета React -- один в директории приложения, а другой в директории вашей библиотеки. При условии, что `myapp` и `mylib` -- директории, находящиеся на одном уровне, выполнение `npm link ../myapp/node_modules/react` из-под директории `mylib` может помочь вам. Это должно заставить библиотеку использовать React-копию из приложения.
