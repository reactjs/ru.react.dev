...
author: Lauren Tan и Mofei Zhang
date: 2025/04/21
description: Мы выпускаем первый Release Candidate (RC) компилятора сегодня.

---

21 апреля 2025 г. [Lauren Tan](https://x.com/potetotes) и [Mofei Zhang](https://x.com/zmofei).
...
2. Мы объединяем `eslint-plugin-react-compiler` с `eslint-plugin-react-hooks`.
3. Мы добавили поддержку swc и работаем с oxc над поддержкой сборки без Babel.

---

[React Compiler](https://react.dev/learn/react-compiler) — это инструмент сборки, который оптимизирует ваше React-приложение с помощью автоматического мемоизирования. В прошлом году мы опубликовали [первую бета-версию](https://react.dev/blog/2024/10/21/react-compiler-beta-release) React Compiler и получили много ценных отзывов и предложений. Мы воодушевлены успехами, достигнутыми теми, кто внедрил компилятор (см. примеры использования от [Sanity Studio](https://github.com/reactwg/react-compiler/discussions/33) и [Wakelet](https://github.com/reactwg/react-compiler/discussions/52)), и работаем над стабильным выпуском.

Сегодня мы выпускаем первый Release Candidate (RC) компилятора. RC представляет собой стабильную и почти финальную версию компилятора, которую безопасно использовать в продакшене.
...
В рамках RC мы упростили добавление React Compiler в ваши проекты и оптимизировали генерацию мемоизации компилятором. React Compiler теперь поддерживает optional chains и индексы массивов в качестве зависимостей. Мы исследуем способы вывода еще большего количества зависимостей, таких как проверки на равенство и интерполяция строк. Эти улучшения в конечном итоге приводят к меньшему количеству повторных рендеров и более отзывчивым пользовательским интерфейсам.

Мы также услышали от сообщества, что проверка `ref` в рендере иногда дает ложные срабатывания. Поскольку наша общая философия заключается в том, чтобы вы могли полностью доверять сообщениям об ошибках и подсказкам компилятора, мы временно отключаем эту проверку по умолчанию. Мы продолжим работать над улучшением этой проверки и повторно включим ее в следующем выпуске.
...
В течение периода RC мы призываем всех пользователей React попробовать компилятор и оставить свои отзывы в репозитории React. Пожалуйста, [откройте issue](https://github.com/facebook/react/issues), если вы столкнетесь с какими-либо ошибками или неожиданным поведением. Если у вас есть общий вопрос или предложение, пожалуйста, опубликуйте их в [React Compiler Working Group](https://github.com/reactwg/react-compiler/discussions).
...
Пользователи Vite могут продолжать использовать [vite-plugin-react](https://github.com/vitejs/vite-plugin-react) для включения компилятора, добавив его как [плагин Babel](https://react.dev/learn/react-compiler#usage-with-vite). Мы также работаем с командой [oxc](https://oxc.rs/) над [добавлением поддержки компилятора](https://github.com/oxc-project/oxc/issues/10048). Как только [rolldown](https://github.com/rolldown/rolldown) будет официально выпущен и будет добавлена поддержка oxc для React Compiler, мы обновим документацию с информацией о том, как выполнить миграцию.

## Обновление React Compiler {/*upgrading-react-compiler*/}
React Compiler лучше всего работает, когда применяемое автоматическое мемоизирование предназначено исключительно для повышения производительности. Будущие версии компилятора могут изменить способ применения мемоизации, например, она может стать более гранулярной и точной.

Однако, поскольку код продукта иногда может нарушать [правила React](https://react.dev/reference/rules) способами, которые не всегда обнаруживаются статически в JavaScript, изменение мемоизации иногда может приводить к неожиданным результатам. Например, ранее мемоизированное значение может использоваться в качестве зависимости для `useEffect` где-то в дереве компонентов. Изменение того, как или будет ли это значение мемоизировано, может привести к чрезмерному или недостаточному срабатыванию этого `useEffect`. Хотя мы рекомендуем использовать [useEffect только для синхронизации](https://react.dev/learn/synchronizing-with-effects), в вашем коде могут быть `useEffect`, которые охватывают другие варианты использования, такие как эффекты, которые должны срабатывать только при изменении определенных значений.
...
Если у вас нет хорошего тестового покрытия, мы рекомендуем закрепить компилятор за конкретной версией (например, `19.1.0`), а не за диапазоном SemVer (например, `^19.1.0`). Вы можете сделать это, передав флаги `--save-exact` (npm/pnpm) или `--exact` (yarn) при обновлении компилятора. Затем вы должны обновлять компилятор вручную, внимательно проверяя, что ваше приложение по-прежнему работает должным образом.

## Дорожная карта к стабильному выпуску {/*roadmap-to-stable*/}
*Это не финальная дорожная карта, и она может быть изменена.*

После периода получения окончательных отзывов от сообщества по RC мы планируем выпустить стабильную версию компилятора.

* ✅ Экспериментальный: Выпущен на React Conf 2024, в основном для получения отзывов от разработчиков приложений.
* ✅ Публичная бета-версия: Доступна сегодня для получения отзывов от авторов библиотек.
* ✅ Release Candidate (RC): React Compiler работает для большинства приложений и библиотек, соответствующих правилам, без проблем.
* Общая доступность: После периода получения окончательных отзывов от сообщества.

После стабильного выпуска мы планируем добавить больше оптимизаций и улучшений компилятора. Это включает как постоянные улучшения автоматического мемоизирования, так и совершенно новые оптимизации, с минимальными или нулевыми изменениями в коде продукта. Каждый апгрейд будет продолжать улучшать производительность и обеспечивать лучшую обработку разнообразных шаблонов JavaScript и React.
