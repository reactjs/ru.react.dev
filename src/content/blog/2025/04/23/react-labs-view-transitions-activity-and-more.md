---
title: "React Labs: Переходы представлений, активность и многое другое"
author: Ricky Hanlon
date: 2025/04/23
description: В публикациях React Labs мы пишем о проектах, находящихся в стадии
  активных исследований и разработок. В этой публикации мы расскажем о двух
  новых экспериментальных возможностях, готовых к тестированию сегодня, а также
  об обновлениях в других областях, над которыми мы работаем сейчас.
---
23 апреля 2025 г. от [Ricky Hanlon](https://twitter.com/rickhanlonii)

---

<Intro>

В публикациях React Labs мы пишем о проектах, находящихся в активной стадии исследований и разработок. В этой публикации мы представляем две новые экспериментальные функции, готовые к тестированию, а также обновления по другим областям, над которыми мы сейчас работаем.
...
React Conf 2025 запланирована на 7–8 октября в Хендерсоне, Невада!

Мы ищем докладчиков, которые помогут нам создать выступления о функциях, описанных в этой публикации. Если вы заинтересованы в выступлении на ReactConf, [подайте заявку здесь](https://forms.reform.app/react-conf/call-for-speakers/) (предварительное предложение доклада не требуется).
...
## Переходы между видами {/*view-transitions*/}

Переходы между видами в React — это новая экспериментальная функция, которая упрощает добавление анимаций при переходе между состояниями пользовательского интерфейса в вашем приложении. По сути, эти анимации используют новый API [`startViewTransition`](https://developer.mozilla.org/ru/docs/Web/API/Document/startViewTransition), доступный в большинстве современных браузеров.
...
Этот новый компонент позволяет декларативно определить, "что" будет анимировано при активации анимации.

Вы можете определить, "когда" анимировать, используя один из следующих трех триггеров для перехода между видами:
...
Когда DOM обновляется из-за триггера анимации — например, `startTransition`, `useDeferredValue` или переключение резервного варианта `Suspense` на контент — React будет использовать [декларативные эвристики](/reference/react/ViewTransition#viewtransition), чтобы автоматически определить, какие компоненты `<ViewTransition>` следует активировать для анимации. Затем браузер выполнит анимацию, определенную в CSS.

Если вы знакомы с API переходов между видами в браузере и хотите узнать, как React его поддерживает, ознакомьтесь с разделом [Как работает `<ViewTransition>`](/reference/react/ViewTransition#how-does-viewtransition-work) в документации.

В этой статье мы рассмотрим несколько примеров использования переходов между видами.
...
export function Router({ children }) {
  ...
  function navigate(url) {
    // Обновляем состояние роутера в процессе перехода.
    startTransition(() => {
      go(url);
    });
  }
...
<Note>
	<p>
		Этот пример использует экспериментальные функции React.
		Чтобы использовать его, вам нужно установить
		<code>react</code> и <code>react-dom</code> в версии
		<code>experimental</code>.
	</p>
	<p>
		Вы можете попробовать этот пример в
		<a href="https://codesandbox.io/s/reactjs-org-example-forked-2222k?file=/src/App.js">
			CodeSandbox
		</a>
		.
	</p>
...
В нашем примере приложения выше обратите внимание, что уже есть анимации при нажатии на кнопку «лайк» и в мерцании запасного варианта Suspense. Это хорошие варианты использования для CSS-анимаций, поскольку они анимируют конкретный элемент.

</Note>

### Анимация переходов {/*animating-navigations*/}

Наше приложение включает роутер с поддержкой Suspense, где [переходы между страницами уже помечены как Transitions](/reference/react/useTransition#building-a-suspense-enabled-router), что означает, что навигация выполняется с помощью `startTransition`:
...
`startTransition` является триггером для View Transition, поэтому мы можем добавить `<ViewTransition>` для анимации между страницами:
...
По умолчанию View Transitions включают стандартную анимацию браузера cross-fade (перекрестное затухание). Добавив это в наш пример, мы получим cross-fade при каждом переходе между страницами:
...
Поскольку наш роутер уже обновляет маршрут с помощью `startTransition`, это однострочное изменение для добавления `<ViewTransition>` активирует его с анимацией перехода по умолчанию.

Если вам интересно, как это работает, см. документацию [Как работает `<ViewTransition>`?](/reference/react/ViewTransition#how-does-viewtransition-work)

<Note>

#### Отказ от анимаций `<ViewTransition>` {/*opting-out-of-viewtransition-animations*/}
...
На практике навигация должна осуществляться через пропсы "enter" и "exit" или с использованием типов переходов (Transition Types).

</Примечание>

### Настройка анимаций {/*customizing-animations*/}

По умолчанию `<ViewTransition>` включает стандартное перекрестное затухание (cross-fade) из браузера.
...
Например, мы можем замедлить анимацию перекрестного затухания `default`:
...
И определить `slow-fade` в CSS, используя [классы view transition](/reference/react/ViewTransition#view-transition-classes):
...
См. [Стилизация переходов представления](/reference/react/ViewTransition#styling-view-transitions) для полного руководства по стилизации `<ViewTransition>`.

### Переходы с общими элементами {/*shared-element-transitions*/}
...
export default function App() {
  ...
  // Это позволяет контенту, не участвующему в переходе с общими элементами,
  // плавно переходить.
  return (
    <ViewTransition default="slow-fade">
      {url === "/" ? <Home /> : <Details />}
...
export default function Page({ heading, children }) {
  ...
  return (
    <div className="page">
      ...
      {/* Отключаем ViewTransition для контента. */}
      {/* Контент может определять свой собственный ViewTransition. */}
      <ViewTransition default="none">
        <div className="bottom">
          <div className="content">{children}</div>
...
export function Thumbnail({ video, children }) {
  // Добавляем имя для анимации с помощью перехода с общими элементами.
...
С помощью типов переходов вы можете предоставлять пользовательские анимации через пропсы для `<ViewTransition>`. Давайте добавим переход с общим элементом для заголовка «6 видео» и «Назад»:
...
Здесь мы передаем пропс `share`, чтобы определить, как анимировать в зависимости от типа перехода. Когда переход с общим элементом активируется из `nav-forward`, применяется класс перехода представления `slide-forward`. Когда он происходит из `nav-back`, активируется анимация `slide-back`. Определим эти анимации в CSS:
...
export function Router({ children }) {
  ...
  useEffect(() => {
    function handlePopState() {
      // Анимация не должна происходить, так как восстановление должно быть синхронным.
      // Даже если это переход.
      startTransition(() => {
        setRouterState({
          url: document.location.pathname + document.location.search,
...
Добавив это, запасной вариант будет плавно переходить в контент. Нажмите на видео, и вы увидите, как информация о видео анимируется:
...
export default function App() {
  ...
  // Анимация медленного затухания по умолчанию.
  return (
    <ViewTransition default="slow-fade">
      {url === "/" ? <Home /> : <Details />}
    </ViewTransition>
...
function VideoDetails({id}) {
  // Плавное затухание запасного варианта в контент.
  return (
    <ViewTransition default="slow-fade">
      <Suspense fallback={<VideoInfoFallback />}>
          <VideoInfo id={id} />
      ...
      <svg
        ...
        {liked ? (
          ...
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="m12 5.184-.808-.771-.004-.004C11.065 4.299 8.522 2.003 6 2.003c-3.736 0-6 2.558-6 6.677 0 4.47 5.471 9.848 10 13.079.602.43 1.187.82 1.74 1.167A.497.497 0 0 0 12 23v-.003c.09 0 .182-.026.26-.074C16.977 19.97 24 13.737 24 8.677 24 4.557 21.743 2 18 2c-.572 0-1.087.174-1.575.472-.67.412-1.148.82-1.266.92-.03.025-.037.031-.019.014l-.013.013L12 7.949 9.832 5.88a10.08 10.08 0 0 0-1.33-.977C7.633 4.367 6.728 4.003 6 4.003c-1.388 0-2.312.459-2.91 1.128C2.466 5.826 2 6.974 2 8.68c0 1.726 1.28 4.058 3.575 6.563 1.948 2.127 4.352 4.078 6.423 5.466z"
            fill="currentColor"
          />
        )}
      </svg>
...
::view-transition-old(.slide-down) { 
  /* Сдвинуть резервный вариант вниз */
  animation: ...;
}

::view-transition-new(.slide-up) {
  /* Сдвинуть контент вверх */
  animation: ...;
}
```
      ...
      <svg
        ...
        {liked ? (
          ...
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="m12 5.184-.808-.771-.004-.004C11.065 4.299 8.522 2.003 6 2.003c-3.736 0-6 2.558-6 6.677 0 4.47 5.471 9.848 10 13.079.602.43 1.187.82 1.74 1.167A.497.497 0 0 0 12 23v-.003c.09 0 .182-.026.26-.074C16.977 19.97 24 13.737 24 8.677 24 4.557 21.743 2 18 2c-.726 0-1.63.364-2.5.9-.67.412-1.148.82-1.266.92-.03.025-.037.031-.019.014l-.013.013L12 7.949 9.832 5.88a10.08 10.08 0 0 0-1.33-.977C7.633 4.367 6.728 4.003 6 4.003c-1.388 0-2.312.459-2.91 1.128C2.466 5.826 2 6.974 2 8.68c0 1.726 1.28 4.058 3.575 6.563 1.948 2.127 4.352 4.078 6.423 5.466z"
            fill="currentColor"
          />
        )}
      </svg>
...
Вы также можете использовать `<ViewTransition>` для анимации списков элементов при их переупорядочивании, например, в списке элементов с возможностью поиска:
...
export default function App() {
  ...
  // Анимация по умолчанию с медленным затуханием.
  return (
    <ViewTransition default="slow-fade">
      {url === "/" ? <Home /> : <Details />}
    </ViewTransition>
...
function VideoDetails({id}) {
  // Анимация от запасного варианта Suspense до контента
  return (
    <Suspense
      fallback={
...
Добавив несколько компонентов `<ViewTransition>` и несколько строк CSS, мы смогли добавить все анимации, показанные выше, в конечный результат.

Мы с энтузиазмом относимся к View Transitions и считаем, что они выведут на новый уровень приложения, которые вы сможете создавать. Они готовы к использованию уже сегодня в экспериментальном канале релизов React.

Давайте уберём медленное затухание и посмотрим на конечный результат:
...
export default function App() {
  ...
  // Анимируем страницы с помощью перекрестного затухания.
  return (
    <ViewTransition key={url}>
      {url === '/' ? <Home /> : <Details />}
    </ViewTransition>
...
function VideoDetails({id}) {
  // Анимируем от запасного варианта Suspense до контента
  return (
    <Suspense
      fallback={
...
Когда `<Activity>` <CodeStep step={1}>видим</CodeStep>, он рендерится как обычно. Когда `<Activity>` <CodeStep step={2}>скрыт</CodeStep>, он размонтируется, но сохранит своё состояние и продолжит рендериться с более низким приоритетом, чем всё видимое на экране.
...
Рассмотрим несколько примеров, улучшающих примеры View Transition выше.

<Note>

**Эффекты не монтируются, когда `<Activity>` скрыт.**

Когда `<Activity>` имеет режим `hidden`, эффекты размонтируются. Концептуально компонент размонтируется, но React сохраняет его состояние для последующего использования.

На практике это работает ожидаемо, если вы следовали руководству [Вам может не понадобиться эффект](/learn/you-might-not-need-an-effect). Чтобы быстро находить проблемные эффекты, мы рекомендуем добавить [`<StrictMode>`](/reference/react/StrictMode), который будет принудительно выполнять размонтирование и монтирование `<Activity>`, чтобы выявить любые неожиданные побочные эффекты.
...
Однако это означает, что если пользователь вернётся на старую страницу, всё предыдущее состояние будет потеряно. Например, если на странице `<Home />` есть поле `<input>`, когда пользователь покидает страницу, `<input>` размонтируется, и весь введённый им текст будет потерян.

Activity позволяет сохранять состояние при смене страниц, чтобы пользователь мог продолжить с того места, где остановился. Это достигается путём оборачивания части дерева в `<Activity>` и переключения `mode`:
...
С этим изменением мы можем улучшить наш пример View Transitions выше. Раньше, когда вы искали видео, выбирали его и возвращались, ваш фильтр поиска терялся. С Activity ваш фильтр поиска восстанавливается, и вы можете продолжить с того места, где остановились.
    ...
    function handlePopState() {
      ...
      startTransition(() => {
        setRouterState({
          url: document.location.pathname + document.location.search,
          pendingNav() {
            // Ничего не делаем. URL уже обновлён.
          },
        });
      });
    }
...
Иногда вы можете захотеть заранее подготовить следующую часть пользовательского интерфейса, которую пользователь, вероятно, будет использовать, чтобы она была готова к моменту, когда он решит ею воспользоваться. Это особенно полезно, если следующий маршрут должен приостановиться в ожидании данных, необходимых для рендеринга, поскольку вы можете помочь обеспечить получение данных до того, как пользователь перейдет на него.

Например, наше приложение в настоящее время должно приостанавливаться для загрузки данных для каждого видео при его выборе. Мы можем улучшить это, отрендерив все страницы в скрытом `<Activity>` до тех пор, пока пользователь не перейдет на них:
...
С этим обновлением, если контент на следующей странице успеет предварительно отрендериться, он будет анимирован без использования запасного варианта `Suspense`. Нажмите на видео и заметьте, что заголовок и описание видео на странице с деталями отображаются немедленно, без запасного варианта:
...
export function Router({ children }) {
  ...
  function navigateBack(url) {
    startTransition(() => {
      // Transition type for the cause "nav back"
      addTransitionType('nav-back');
      go(url);
    });
  }
...
Для частей UI, отрендеренных с `mode="visible"`, React будет снижать приоритет гидратации контента внутри Activity, аналогично тому, как контент Suspense гидратируется с более низким приоритетом. Если пользователь взаимодействует со страницей, мы будем отдавать приоритет гидратации внутри границы при необходимости.

Это продвинутые сценарии использования, но они демонстрируют дополнительные преимущества, учтённые в Activity.

### Будущие режимы для Activity {/*future-modes-for-activity*/}

В будущем мы можем добавить новые режимы для Activity.

Например, распространённый сценарий использования — рендеринг модального окна, где предыдущая «неактивная» страница видна за «активным» модальным представлением. Режим «hidden» не подходит для этого сценария, поскольку он не видим и не включён в SSR.

Вместо этого мы рассматриваем новый режим, который будет сохранять контент видимым — и включённым в SSR — но оставлять его размонтированным и снижать приоритет обновлений. Этот режим также может потребоваться для «приостановки» обновлений DOM, поскольку может отвлекать видеть фоновый контент, обновляющийся во время открытия модального окна.
...
Это области, которые мы всё ещё исследуем, и мы поделимся дополнительной информацией по мере продвижения. Для получения дополнительной информации о том, что включает в себя Activity сегодня, [ознакомьтесь с документацией](/reference/react/Activity).

---

# Функции в разработке {/*features-in-development*/}

Мы также разрабатываем функции, которые помогут решить перечисленные ниже распространённые проблемы.
...
Когда решения, над которыми мы работаем, публикуются слишком рано, это может вызвать суматоху и путаницу в сообществе. Чтобы сбалансировать прозрачность и ограничить путаницу, мы делимся проблемами, для которых мы в настоящее время разрабатываем решения, не раскрывая конкретное решение, которое у нас есть на уме.

По мере прогресса этих функций мы будем анонсировать их в блоге с включённой документацией, чтобы вы могли их опробовать. 
...
Эта функция все еще находится в разработке, поэтому мы еще не готовы опубликовать документацию для полного выпуска в качестве экспериментальной функции. Вы можете получить предварительный просмотр, используя экспериментальную версию React, которая автоматически добавит треки производительности в профили:
...
Существует несколько известных проблем, которые мы планируем устранить, таких как производительность, и трек планировщика не всегда "соединяет" работу между приостановленными деревьями, поэтому он еще не готов к тестированию. Мы также продолжаем собирать отзывы от ранних пользователей, чтобы улучшить дизайн и удобство использования треков.

Как только мы решим эти проблемы, мы опубликуем экспериментальную документацию и сообщим, что она готова к тестированию.

---

## Автоматические зависимости эффектов {/*automatic-effect-dependencies*/}

Когда мы выпустили хуки, у нас было три мотива:
...
- **Думать в терминах функций, а не жизненных циклов**: хуки позволяют разделить один компонент на более мелкие функции на основе того, какие части связаны (например, настройка подписки или получение данных), вместо того, чтобы принудительно разделять по методам жизненного цикла.
...
Мы провели некоторое время, исследуя, почему эффекты воспринимаются с точки зрения компонента, и мы думаем, что одна из причин — это массив зависимостей. Поскольку вам приходится его писать, он прямо перед вами и напоминает вам, на что вы "реагируете", и подталкивает вас к ментальной модели "сделай это, когда эти значения изменятся".
...
С этим кодом React Compiler может вывести зависимости за вас и вставить их автоматически, чтобы вам не нужно было их видеть или писать. С такими функциями, как [расширение IDE](#compiler-ide-extension) и [`useEffectEvent`](/reference/react/experimental_useEffectEvent), мы можем предоставить CodeLens, чтобы показать вам, что компилятор вставил для случаев, когда вам нужно отладить или оптимизировать, удалив зависимость. Это помогает укрепить правильную ментальную модель для написания эффектов, которые могут запускаться в любое время для синхронизации состояния вашего компонента или хука с чем-то еще.

Мы надеемся, что автоматическая вставка зависимостей не только упростит написание, но и сделает их более понятными, заставляя вас думать о том, что делает эффект, а не о жизненных циклах компонентов.

---

## Расширение IDE для компилятора {/*compiler-ide-extension*/}

Ранее на этой неделе [мы поделились](/blog/2025/04/21/react-compiler-rc) релиз-кандидатом React Compiler, и мы работаем над выпуском первой стабильной версии компилятора по SemVer в ближайшие месяцы.
...
Расширение IDE все еще находится на ранней стадии исследования, но мы поделимся нашим прогрессом в будущих обновлениях.

---

## Рефы для фрагментов {/*fragment-refs*/}

Многие API DOM, такие как API для управления событиями, позиционирования и фокуса, трудно компоновать при написании с использованием React. Это часто приводит к тому, что разработчики прибегают к эффектам, управляя несколькими рефами, используя такие API, как `findDOMNode` (удален в React 19).

Мы исследуем возможность добавления рефов к фрагментам, которые будут указывать на группу DOM-элементов, а не только на один элемент. Мы надеемся, что это упростит управление несколькими дочерними элементами и облегчит написание компонуемого кода React при вызове API DOM.
...
- **Жесты непрерывны**: во время свайпа анимация привязана ко времени размещения пальца, а не запускается и выполняется до завершения.
- **Жесты не завершаются**: когда вы отпускаете палец, анимации жестов могут завершиться или вернуться в исходное состояние (например, когда вы частично открыли меню) в зависимости от того, насколько далеко вы продвинулись.
- **Жесты инвертируют старое и новое**: во время анимации вы хотите, чтобы страница, с которой вы анимируете, оставалась "живой" и интерактивной. Это инвертирует модель View Transition браузера, где "старое" состояние — это снимок, а "новое" состояние — это живой DOM.
...
Когда мы выпустили React 18 с конкурентным рендерингом, мы также выпустили `useSyncExternalStore`, чтобы внешние библиотеки хранилищ, которые не использовали состояние или контекст React, могли [поддерживать конкурентный рендеринг](https://github.com/reactwg/react-18/discussions/70), принудительно синхронизируя рендеринг при обновлении хранилища.

Однако использование `useSyncExternalStore` сопряжено с затратами, поскольку оно принудительно выходит из конкурентных функций, таких как переходы, и заставляет существующий контент отображать резервные варианты Suspense.
