...
Распространенный сценарий использования в приложениях React — это изменение данных с последующим обновлением состояния в ответ. Например, когда пользователь отправляет форму для изменения своего имени, вы сделаете запрос к API, а затем обработаете ответ. В прошлом вам приходилось вручную обрабатывать состояния ожидания, ошибки, оптимистичные обновления и последовательные запросы.

Например, вы могли обрабатывать состояния ожидания и ошибок с помощью `useState`:
...
Асинхронный переход немедленно установит состояние `isPending` в `true`, выполнит асинхронный запрос(ы) и установит `isPending` в `false` после всех переходов. Это позволяет сохранить текущий пользовательский интерфейс отзывчивым и интерактивным во время изменения данных.
...
- **Обработка ошибок**: Actions предоставляют обработку ошибок, чтобы вы могли отображать Error Boundaries при сбое запроса и автоматически откатывать оптимистичные обновления к исходному значению.
...
Подробнее см. [#28491](https://github.com/facebook/react/pull/28491).
...
В дизайн-системах принято создавать компоненты дизайна, которым нужен доступ к информации о `<form>`, в котором они находятся, без необходимости передавать пропсы вниз по дереву компонентов. Это можно сделать через Context, но чтобы упростить распространенный случай, мы добавили новый хук `useFormStatus`:
...
Чтобы исправить это, вам нужно передать промис из библиотеки или фреймворка с поддержкой Suspense, который поддерживает кеширование для промисов. В будущем мы планируем выпустить функции, которые упростят кеширование промисов в процессе рендеринга.

</Note>

Вы также можете читать контекст с помощью `use`, что позволяет условно считывать Context, например, после ранних возвратов:
...
API `use` может быть вызван только в процессе рендеринга, аналогично хукам. В отличие от хуков, `use` может быть вызван условно. В будущем мы планируем поддерживать больше способов потребления ресурсов в процессе рендеринга с помощью `use`.

Для получения дополнительной информации см. документацию по [`use`](/reference/react/use).

## Статические API React DOM {/*new-react-dom-static-apis*/}
...
Когда Server Action определяется с директивой `"use server"`, ваш фреймворк автоматически создаст ссылку на серверную функцию и передаст эту ссылку клиентскому компоненту. Когда эта функция вызывается на клиенте, React отправит запрос на сервер для выполнения функции и вернет результат.
...
Мы также улучшили отчетность об ошибках для ошибок гидратации в `react-dom`. Например, вместо вывода нескольких ошибок в режиме разработки без какой-либо информации о несоответствии:
...
В будущих версиях мы объявим устаревшим вызов рефов с `null` при размонтировании компонентов.

</Note>

Из-за введения функций очистки для рефов, возврат чего-либо еще из колбэка `ref` теперь будет отклоняться TypeScript. Исправление обычно заключается в прекращении использования неявных возвратов, например:
...
Исходный код возвращал экземпляр `HTMLDivElement`, и TypeScript не знал, предназначалось ли это для функции очистки, или вы не хотели возвращать функцию очистки.
...
Таблицы стилей, как внешние ссылки (`<link rel="stylesheet" href="...">`), так и встроенные (`<style>...</style>`), требуют тщательного позиционирования в DOM из-за правил приоритета стилей. Создание функциональности таблиц стилей, допускающей композицию в компонентах, — сложная задача, поэтому пользователи часто либо загружают все свои стили далеко от компонентов, которые могут от них зависеть, либо используют библиотеку стилей, которая инкапсулирует эту сложность.

В React 19 мы решаем эту сложность и обеспечиваем более глубокую интеграцию с конкурентным рендерингом на клиенте и потоковым рендерингом на сервере благодаря встроенной поддержке таблиц стилей. Если вы укажете React `precedence` (приоритет) вашей таблицы стилей, он будет управлять порядком вставки таблицы стилей в DOM и гарантировать, что таблица стилей (если она внешняя) будет загружена до отображения контента, зависящего от этих правил стилей.
...
Во время серверного рендеринга React включит таблицу стилей в `<head>`, что гарантирует, что браузер не будет отрисовывать до ее загрузки. Если таблица стилей обнаружена поздно, после того как мы уже начали потоковую передачу, React гарантирует, что таблица стилей будет вставлена в `<head>` на клиенте перед отображением контента границы Suspense, зависящей от этой таблицы стилей.
...
Для пользователей, привыкших загружать таблицы стилей вручную, это возможность разместить эти таблицы стилей рядом с компонентами, которые от них зависят, что обеспечивает лучшее локальное понимание и упрощает обеспечение загрузки только тех таблиц стилей, которые вам действительно нужны.

Библиотеки стилей и интеграции стилей с бандлерами также могут использовать эту новую возможность, так что даже если вы не рендерите свои таблицы стилей напрямую, вы все равно можете получить выгоду, когда ваши инструменты будут обновлены для использования этой функции.
...
Во всех средах рендеринга асинхронные скрипты будут дедуплицироваться, так что React загрузит и выполнит скрипт только один раз, даже если он рендерится несколькими разными компонентами.
...
React 19 включает ряд новых API для загрузки и предварительной загрузки браузерных ресурсов, чтобы максимально упростить создание отличных интерфейсов, не сдерживаемых неэффективной загрузкой ресурсов.
...
Эти API можно использовать для оптимизации начальной загрузки страниц, перемещая обнаружение дополнительных ресурсов, таких как шрифты, из загрузки таблиц стилей. Они также могут ускорить клиентские обновления, предварительно загружая список ресурсов, используемых ожидаемой навигацией, а затем активно предварительно загружая эти ресурсы при клике или даже при наведении курсора.
...
В React 19 неожиданные теги в `<head>` и `<body>` будут пропущены, избегая ошибок несоответствия. Если React потребуется повторно отрендерить весь документ из-за несвязанного несоответствия гидратации, он оставит на месте таблицы стилей, вставленные сторонними скриптами и расширениями браузера.

### Улучшенная отчетность об ошибках {/*error-handling*/}

Мы улучшили обработку ошибок в React 19, чтобы устранить дублирование и предоставить параметры для обработки пойманных и непойманных ошибок. Например, когда в рендеринге возникает ошибка, пойманная Error Boundary, ранее React выбрасывал ошибку дважды (один раз для исходной ошибки, затем снова после неудачной автоматической коррекции), а затем вызывал `console.error` с информацией о месте возникновения ошибки.
...
- `onRecoverableError`: вызывается, когда выбрасывается ошибка и она автоматически исправляется.
