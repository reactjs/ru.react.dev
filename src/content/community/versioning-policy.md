...
Все стабильные сборки React проходят тщательное тестирование и следуют принципам семантического версионирования (semver). React также предлагает каналы предварительных выпусков для получения ранней обратной связи по экспериментальным функциям. Эта страница описывает, чего можно ожидать от выпусков React.

</Intro>

Эта политика версионирования описывает наш подход к номерам версий для таких пакетов, как `react` и `dom`. Список предыдущих выпусков см. на странице [Версии](/versions).
...
* При выпуске **критических исправлений ошибок** мы выпускаем **патч**, изменяя число **z** (например, 15.6.2 -> 15.6.3).
* При выпуске **новых функций** или **некритических исправлений** мы выпускаем **минорный релиз**, изменяя число **y** (например, 15.6.2 -> 15.7.0).
* При выпуске **обратно несовместимых изменений** мы выпускаем **мажорный релиз**, изменяя число **x** (например, 15.6.2 -> 16.0.0).

Мажорные выпуски также могут содержать новые функции, а любой выпуск может включать исправления ошибок.

Минорные выпуски являются наиболее распространенным типом выпусков.
...
* **Предупреждений в режиме разработки.** Поскольку они не влияют на поведение в продакшене, мы можем добавлять новые предупреждения или изменять существующие между мажорными версиями. Фактически, именно это позволяет нам надежно предупреждать о предстоящих обратно несовместимых изменениях.
* **API, начинающихся с `unstable_`.** Они предоставляются как экспериментальные функции, API которых мы еще не уверены. Выпуская их с префиксом `unstable_`, мы можем быстрее итерировать и быстрее достичь стабильного API.
* **Альфа- и Canary-версий React.** Мы предоставляем альфа-версии React как способ раннего тестирования новых функций, но нам нужна гибкость для внесения изменений на основе того, что мы узнаем в альфа-период. Если вы используете эти версии, имейте в виду, что API могут измениться до стабильного выпуска.
* **Недокументированных API и внутренних структур данных.** Если вы обращаетесь к внутренним именам свойств, таким как `__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED` или `__reactInternalInstance$uk43rzhitjg`, гарантии нет. Вы сами себе хозяин.

Эта политика разработана прагматично: мы, конечно, не хотим доставлять вам неприятности. Если бы мы увеличивали номер мажорной версии для всех этих изменений, мы бы выпускали больше мажорных версий и в конечном итоге причиняли бы больше проблем с версионированием сообществу. Это также означало бы, что мы не сможем так быстро развиваться в улучшении React, как нам хотелось бы.
...
Возможно, что минорный выпуск не будет включать новых функций. [Это разрешено semver](https://semver.org/#spec-item-7), который гласит: **"[минорная версия] МОЖЕТ быть увеличена, если внутри приватного кода введена существенная новая функциональность или улучшения. Она МОЖЕТ включать изменения на уровне патчей."**

Однако это вызывает вопрос, почему такие выпуски не версионируются как патчи.

Ответ заключается в том, что любое изменение в React (или другом программном обеспечении) несет в себе некоторый риск неожиданных сбоев. Представьте себе сценарий, когда патч-релиз, исправляющий одну ошибку, случайно вводит другую. Это было бы не только нарушением для разработчиков, но и подорвало бы их уверенность в будущих патч-релизах. Это особенно прискорбно, если исходное исправление касается ошибки, которая редко встречается на практике.

У нас довольно хороший послужной список в плане выпуска React без ошибок, но патч-релизы имеют еще более высокую планку надежности, поскольку большинство разработчиков предполагают, что их можно использовать без негативных последствий.
...
React полагается на активное сообщество open source для сообщения об ошибках, открытия pull request и [отправки RFC](https://github.com/reactjs/rfcs). Чтобы стимулировать обратную связь, мы иногда делимся специальными сборками React, которые включают невыпущенные функции.
...
- [**Canary**](#canary-channel) отслеживает основную ветку репозитория исходного кода React. Рассматривайте их как релиз-кандидатов для следующего выпуска по semver. **[Фреймворки или другие курируемые настройки могут выбрать использование этого канала с закрепленной версией React.](/blog/2023/05/03/react-canaries) Вы также можете использовать Canary для интеграционного тестирования между React и сторонними проектами.**
...
* Выпуски Canary [должны быть закреплены](/blog/2023/05/03/react-canaries) и могут содержать обратно несовместимые изменения. Они предназначены для курируемых настроек (например, фреймворков), которые хотят постепенно выпускать новые функции и исправления ошибок React по своему собственному графику выпуска.
...
Latest — это канал, используемый для стабильных выпусков React. Он соответствует тегу `latest` в npm. Это рекомендуемый канал для всех приложений React, которые отправляются реальным пользователям.

**Если вы не уверены, какой канал использовать, выбирайте Latest.** Если вы используете React напрямую, это то, что вы уже используете. Вы можете ожидать, что обновления для Latest будут чрезвычайно стабильными. Версии следуют схеме семантического версионирования, как [описано ранее.](#stable-releases)
...
Степень изменений между последним выпуском Canary и последним выпуском Latest примерно такая же, как между двумя минорными версиями semver. Однако **канал Canary не соответствует семантическому версионированию.** Вы можете ожидать случайных обратно несовместимых изменений между последовательными выпусками в канале Canary.
...
Выпуски в Canary публикуются с тегом `canary` в npm. Версии генерируются на основе хэша содержимого сборки и даты коммита, например `18.3.0-canary-388686f29-20230503`.
...
Все изменения в React проходят тщательное внутреннее тестирование перед выпуском для общественности. Однако существует множество сред и конфигураций, используемых во всей экосистеме React, и мы не можем протестировать их все.

Если вы являетесь автором стороннего фреймворка React, библиотеки, инструмента разработки или подобного инфраструктурного проекта, вы можете помочь нам сохранить стабильность React для ваших пользователей и всего сообщества React, периодически запуская свой набор тестов против последних изменений. Если вы заинтересованы, выполните следующие шаги:

- Настройте cron-задание, используя предпочтительную платформу непрерывной интеграции. Cron-задания поддерживаются как [CircleCI](https://circleci.com/docs/2.0/triggers/#scheduled-builds), так и [Travis CI](https://docs.travis-ci.com/user/cron-jobs/).
- В cron-задании обновите ваши пакеты React до последнего выпуска React в канале Canary, используя тег `canary` в npm. Используя npm cli:
...
- Если что-то неожиданно сломается, пожалуйста, сообщите нам, [создав issue](https://github.com/facebook/react/issues).
...
Обычно обновление Canary сопровождается соответствующим обновлением Experimental. Они основаны на одном и том же исходном ревизии, но собраны с использованием другого набора флагов функций.
...
Выпуски в Experimental публикуются с тегом `experimental` в npm. Версии генерируются на основе хэша содержимого сборки и даты коммита, например `0.0.0-experimental-68053d940-20210623`.

#### Что входит в экспериментальный выпуск? {/*what-goes-into-an-experimental-release*/}

Экспериментальные функции — это те, которые еще не готовы к выпуску для широкой публики и могут кардинально измениться до их окончательного утверждения. Некоторые эксперименты могут никогда не быть завершены — причина, по которой у нас есть эксперименты, заключается в тестировании жизнеспособности предлагаемых изменений.

Например, если бы канал Experimental существовал, когда мы анонсировали хуки, мы бы выпустили хуки в канал Experimental за несколько недель до того, как они стали доступны в Latest.

Вы можете найти полезным запуск интеграционных тестов против Experimental. Это на ваше усмотрение. Однако имейте в виду, что Experimental еще менее стабилен, чем Canary. **Мы не гарантируем никакой стабильности между экспериментальными выпусками.**
...
Вы всегда можете обратиться к [истории](https://github.com/facebook/react/commits/main) нашего общедоступного репозитория GitHub для получения полного списка изменений.