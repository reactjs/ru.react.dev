...
* начинаются со слова `handle`, после которого следует название события.
...
Переданный инлайн код, как этот, не сработает по клику - он будет срабатывать каждый раз при рендеринге компонента:
...
Если вы используете [дизайн систему](https://uxdesign.cc/everything-you-need-to-know-about-design-systems-54b109851969), то обычно компоненты, такие как кнопки, содержат стили, но не определяют поведение. Поэтому такие компоненты, как `PlayButton` и `UploadButton` будут передавать обработчики событий вниз.

### Naming event handler props {/*naming-event-handler-props*/}

Встроенные компоненты, такие как `<button>` и `<div>`, поддерживают только [названия событий браузера](/reference/react-dom/components/common#common-props), такие как `onClick`. Но, когда вы создаете свои собственные компоненты, вы можете называть их обработчики событий как угодно.
...
В примере выше `<button onClick={onSmash}>` показывает, что браузеру `<button>` (строчная буква) все еще нужен реквизит `onClick`, но имя реквизита, полученное вашим пользовательским компонентом `Button`, зависит от вас!
